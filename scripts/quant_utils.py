...
class ActQuantWrapper(torch.nn.Module):
    ...
    def extra_repr(self) -> str:
        ...
        if self.online_full_had or self.online_full_hadnopow2 or self.online_partial_had or self.online_random_had:
            str_ += f'\n'+'online rotated'
        return str_
...
def find_qlayers_exclude(module, layers=[torch.nn.Linear,
                                ActQuantWrapper], name='', exclude_name=''):
    if type(module) in layers:
        return {name: module}
    res = {}
    for name1, child in module.named_children():
        if exclude_name in name1:
            continue
        res.update(find_qlayers_exclude(
            child, layers=layers, name=name + '.' + name1 if name != '' else name1, exclude_name=exclude_name
        ))
    return res
